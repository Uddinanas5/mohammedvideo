...
export const GlowReveal: React.FC<{ scene: Scene }> = ({ scene }) => {
  ...
  const renderWord = (w: { word: string; start: number }, i: number) => {
    ...
    const glowIntensity = interpolate(revealProgress, [0, 0.5, 1], [0, 1, 0.6]);
    ...
    return (
      <span
        ...
        style={{
          ...
          textShadow: `0 0 ${30 * glowIntensity}px rgba(200,220,255,0.8), 0 0 ${60 * glowIntensity}px rgba(100,150,255,0.4), 0 4px 20px rgba(0,0,0,0.6)`,
        }}
      >
        {w.word}
      </span>
